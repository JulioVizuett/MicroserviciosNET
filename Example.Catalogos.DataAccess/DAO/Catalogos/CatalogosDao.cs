...
namespace Example.Catalogos.DataAccess.DAO.Catalogos
{
    ...
    public class CatalogosDao : ICatalogosDao
    {
        ...
        {
            return ListaBaseDatos.ListaDatos;
        }

        /// <inheritdoc/>
        public async Task<CatalogosModel> GetCatalogosAsync(int id)
        {
            return ListaBaseDatos.ListaDatos.FirstOrDefault(p => p.Id.Equals(id));
        }

        /// <inheritdoc/>
        public async Task<bool> InsertCatalogos(CatalogosModel model)
        {
            ListaBaseDatos.ListaDatos.Add(model);
            return true;
        }

        /// <inheritdoc/>
        public async Task<bool> ActualizarDatos(CatalogosModel catalogosModel) {
            CatalogosModel user = ListaBaseDatos.ListaDatos.FirstOrDefault(p => p.Id.Equals(catalogosModel.Id));
        ...
        public async Task<bool> BorrarRegistro(int id) {
            CatalogosModel user = ListaBaseDatos.ListaDatos.FirstOrDefault(p => p.Id.Equals(id));
            return ListaBaseDatos.ListaDatos.Remove(user);
        }
    }
}
